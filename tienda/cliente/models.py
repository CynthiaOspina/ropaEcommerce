...
def user_loader(user_id):
    return Registro.query.get(user_id)
...
class Registro(db.Model, UserMixin):
    ...
    password = db.Column(db.Binary(200), unique=False)
    ...
    profile = db.Column(db.String(50), unique=False)
    date_created = db.Column(db.DateTime, nullable=False, default=datetime.now())

    def __repr__(self):
        return '<Registro %r>' % self.name
...
class JsonEcodeDict(db.TypeDecorator):
    ...
    def get_valu(self, value, dialect):
        if value is None:
            return '{}'
        else:
            return json.loads(value)

class OrdenCliente(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    factura = db.Column(db.String(20), unique=True, nullable=False)
    ...
    def __repr__(self):
        return '<OrdernCliente %r>' % self.factura
